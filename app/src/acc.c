...
static void acc_read_data(struct k_work *work)
{
    ...
    if (err && err != -EACCES)
    {
        LOG_WRN("ACC notify failed (%d)", err);
        return;
    }
...
int acc_start(void)
{
    ...
    err = gpio_pin_interrupt_configure(acc_int_port, ACC_INT_PIN, GPIO_INT_EDGE_TO_ACTIVE);
    if (err)
    {
        LOG_ERR("Failed to configure accelerometer sensor int pin interrupt");
        sensor_i2c_lock();
        (void)acc_sensor_stop(&i2c);
        sensor_i2c_unlock();
        return err;
    }
...
int acc_stop(void)
{
    ...
    int err = gpio_pin_interrupt_configure(acc_int_port, ACC_INT_PIN, GPIO_INT_DISABLE);
    if (err)
    {

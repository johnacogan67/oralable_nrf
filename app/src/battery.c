...
uint8_t battery_voltage_to_percent(int32_t voltage_mv)
{
    ...
    if (voltage_mv >= 3650) return 45;
    if (voltage_mv >= 3600) return 40;
    if (voltage_mv >= 3550) return 35;
    if (voltage_mv >= 3500) return 30;
    if (voltage_mv >= 3450) return 25;
    if (voltage_mv >= 3400) return 20;
    if (voltage_mv >= 3350) return 15;
    if (voltage_mv >= 3300) return 10;
    if (voltage_mv >= 3200) return 5;
    return 0;
}
